...
class ScCalendar:
    ...
    def __init__(self, year, months=[], firstDay=calendar.SUNDAY, drawSauce=True, sepMonths='/', lang='English', showWeekNumber=1, events = []):
        """ Setup basic things """
        # params
        ...
        self.cStyleDate = "cStyleDate" # style for ordinary date of month
        self.pStyleDate = "pStyleDate"
        self.cStyleWeekHolyday = "cStyleWeekHolyday" # style for the Sunday
        self.pStyleWeekHolyday = "pStyleWeekHolyday"
        self.cStyleSaturday = "cStyleSaturday"   # Style for the Saturday
        self.pStyleSaturday = "pStyleSaturday"
        self.cStyleHolyDate = "cStyleHolyDate" # style for holydays in a month
        self.pStyleHolyDate = "pStyleHolyDate"
        self.cStyleWeekday = "cStyleWeekday" # style for the name of day of the week (working days)
        self.pStyleWeekday = "pStyleWeekday"
        self.cStyleMonth = "cStyleMonth" # style for the name of the month
        self.pStyleMonth = "pStyleMonth"
        self.cStyleEvent = "cStyleEvent" # style fot the events
        self.pStyleEvent = "pStyleEvent"
        self.cStylePrevNextMonth = "cStylePrevNextMonth" # Style for days of prev. or next month occuring on the cur. month
        self.pStylePrevNextMonth = "pStylePrevNextMonth"
        self.cStylePrevNextMonthSD = "cStylePrevNextMonthSD"
        self.pStylePrevNextMonthSD = "pStylePrevNextMonthSD"
        self.cStylePrevNextMonthHD = "cStylePrevNextMonthHD" # Style for days of prev. or next month occuring on the cur. month HD
        self.pStylePrevNextMonthHD = "pStylePrevNextMonthHD"
        self.cStyleWeekNo = "cStyleWeekNo"   # Style for the week number column
        self.pStyleWeekNo = "pStyleWeekNo"

        self.masterPage = "Weekdays"
        self.sepMonths = sepMonths

        # Some default variables
        self.defaultFont = "Palladio Uralic Regular"
        self.defaultFontSize = 30
        self.defaultColor = "Black"
        self.defaultWeekNoFontSize = 20
        self.defaultWeekNoColor = "Grey 50"

        # Some custom vaiables
        self.saturdayFont = "Palladio Uralic Bold"
        self.sundayFont = "Palladio Uralic Bold"
#        self.monthFont = "Palladio Uralic Italic"
        self.monthFont = "URW Chancery L Medium Italic"
        self.saturdayColor = "Black"
        self.sundayColor = "Red"
        self.monthColor = "Grey50"
        self.monthSize = 50
        self.eventFontSize = 10
        self.eventFontColor = "Red"
    ...
    def createCalendar(self):
        ...
        createCharStyle(name=self.cStyleDate, font=self.defaultFont, fontsize=self.defaultFontSize, fillcolor=self.defaultColor)
        createParagraphStyle(name=self.pStyleDate, alignment=1, charstyle=self.cStyleDate)

        # Saturday
        createCharStyle(name=self.cStyleSaturday, font=self.saturdayFont, fontsize=self.defaultFontSize, fillcolor=self.saturdayColor)
        createParagraphStyle(name=self.pStyleSaturday, alignment=1, charstyle=self.cStyleSaturday)

        # A Holyday (Sunday)
        createCharStyle(name=self.cStyleHolyDate, font=self.defaultFont, fontsize=self.defaultFontSize, fillcolor=self.sundayColor)
        createParagraphStyle(name=self.pStyleHolyDate, alignment=1, charstyle=self.cStyleHolyDate)

        # The name of the week day (Sunday not included)
        createCharStyle(name=self.cStyleWeekday, fillcolor=self.defaultColor, font=self.defaultFont, fontsize=self.defaultFontSize )
        createParagraphStyle(name=self.pStyleWeekday, alignment=1, charstyle=self.cStyleWeekday)

        # A Holyday during the week
        createCharStyle(name=self.cStyleWeekHolyday, font=self.sundayFont, fontsize=self.defaultFontSize, fillcolor=self.sundayColor)
        createParagraphStyle(name=self.pStyleWeekHolyday, alignment=1, charstyle=self.cStyleWeekHolyday)

        # The name of the month
        createCharStyle(name=self.cStyleMonth, font=self.monthFont, fontsize=self.monthSize, fillcolor=self.monthColor)
        createParagraphStyle(name=self.pStyleMonth, alignment=1, charstyle=self.cStyleMonth)

        # Event
        createCharStyle(name=self.cStyleEvent, font=self.defaultFont, fontsize=self.eventFontSize, fillcolor=self.eventFontColor)
        createParagraphStyle(name=self.pStyleEvent, alignment=1, charstyle=self.cStyleEvent)

        # If some days from the previous or next month occur on the current month
        createCharStyle(name=self.cStylePrevNextMonth, font=self.defaultFont, fontsize=self.defaultFontSize, fillcolor=self.defaultColor, fillshade=0.4)
        createParagraphStyle(name=self.pStylePrevNextMonth, alignment=1, charstyle=self.cStylePrevNextMonth)

        # If some days from the previous or next month occur on the current month and they are Saturdays
        createCharStyle(name=self.cStylePrevNextMonthSD, font=self.saturdayFont, fontsize=self.defaultFontSize, fillcolor=self.saturdayColor, fillshade=0.4)
        createParagraphStyle(name=self.pStylePrevNextMonthSD, alignment=1, charstyle=self.cStylePrevNextMonthSD)

        # If some days from the previous or next month occur on the current month and they are Holydays
        createCharStyle(name=self.cStylePrevNextMonthHD, font=self.sundayFont, fontsize=self.defaultFontSize, fillcolor=self.sundayColor, fillshade=0.4)
        createParagraphStyle(name=self.pStylePrevNextMonthHD, alignment=1, charstyle=self.cStylePrevNextMonthHD)

        # Number of week
        createCharStyle(name=self.cStyleWeekNo, font=self.defaultFont, fontsize=self.defaultWeekNoFontSize, fillcolor=self.defaultWeekNoColor)
        createParagraphStyle(name=self.pStyleWeekNo, alignment=1, charstyle=self.cStyleWeekNo)
...
class ScEventCalendar(ScCalendar):
    ...
    def __init__(self, year, months = [], firstDay = calendar.SUNDAY, drawSauce=True, sepMonths='/', lang='English', showWeekNumber=1, events = []):
        ScCalendar.__init__(self, year, months, firstDay, drawSauce, sepMonths, lang, showWeekNumber, events)
...
class ScHorizontalEventCalendar(ScEventCalendar):
    ...
    def __init__(self, year, months = [], firstDay = calendar.SUNDAY, drawSauce=True, sepMonths='/', lang='English', showWeekNumber=1, events = []):
        ScEventCalendar.__init__(self, year, months, firstDay, drawSauce, sepMonths, lang, showWeekNumber, events)
...
class ScVerticalCalendar(ScCalendar):
    ...
    def setupDocVariables(self):
        ...
        # rows and cols
        if self.showWeekNumber == 1:
            self.colNumWeekSize = 30
        else:
            self.colNumWeekSize = 0
        self.rowSize = self.gmean / 8
        self.colSize = (self.width - self.colNumWeekSize) / 7
    ...
    def setupMasterPage(self):
        ...
        if self.showWeekNumber == 1:
            self.test="No"
            cel = createText(self.marginl, self.calHeight + self.rowSize, self.colNumWeekSize, self.rowSize, self.test)
            setText("№", cel)
            setStyle(self.pStyleWeekNo, cel)
            setFillColor("Warm Black", str(self.test))
            setFillShade(2, str(self.test))
            setTextDistances(0,0,15,0)

        for j in self.dayOrder: # days
            cel = createText(self.marginl + self.colNumWeekSize + rowCnt*self.colSize,
                             self.calHeight + self.rowSize,
                             self.colSize, self.rowSize)
            setText(j, cel)

            # When the week starts by Sunday, we should be able to apply the correct styles.
            if self.mycal.firstweekday == calendar.SUNDAY:
                if j == self.dayOrder[0]:
                    setStyle(self.pStyleWeekHolyday,cel)
                    setFillColor("Warm Black")
                    setFillShade(5)
                else:
                ...
                if j == self.dayOrder[6]:
                    setStyle(self.pStyleWeekHolyday,cel)
                    setFillColor("Warm Black")
                    setFillShade(10)
                elif j == self.dayOrder[5]:
                    setStyle(self.pStyleSaturday,cel)
                    setFillColor("Warm Black")
                    setFillShade(5)
                else:
                    setStyle(self.pStyleWeekday, cel)
    ...
    def createHeader(self, monthName):
        ...
        setTextDistances(0,0,10,0,header)
        setStyle(self.pStyleMonth, header)
        setTextScalingH(120, header)
...
class ScClassicCalendar(ScVerticalCalendar):
    ...
    def __init__(self, year, months = [], firstDay = calendar.SUNDAY, drawSauce=True, sepMonths='/', lang='English', showWeekNumber=1, events = []):
        ScVerticalCalendar.__init__(self, year, months, firstDay, drawSauce, sepMonths, lang, showWeekNumber, events)
    ...
    def createMonthCalendar(self, month, cal):
        ...
        for week in cal:
            colCnt = 0
            if self.showWeekNumber == 1:
                cel = createText(self.marginl, self.calHeight + rowCnt * self.rowSize, self.colNumWeekSize, self.rowSize)
                setText(str(week[(calendar.THURSDAY-calendar.firstweekday())%7].isocalendar()[1]), cel)
                setStyle(self.pStyleWeekNo, cel)
                setTextDistances(0,0,10,0)
                setFillColor("Warm Black")
                setFillShade(2)
            for day in week:
                cel = createText(self.marginl + self.colNumWeekSize + colCnt * self.colSize,
                                 self.calHeight + rowCnt * self.rowSize,
                                 self.colSize, self.rowSize)
                colCnt += 1

                # We check whether there is an event for the current day
                self.isevent = 0
                for ev in self.events:
                    if ev == str(datetime.date(day.year, day.month, day.day).isoformat()) and day.month == month + 1:
                        #setText(str(day), cel)
                        setText(str(day.day),cel)
                        insertText(str('\n'), -1, cel)
                        m = re.search('\[vText\(\'(.+?)\'\)\]',str(self.events[ev]))
                        if m:
                            insertText(str(m.group(1)), -1, cel)
                        #insertText(str(self.events[ev]), -1, cel)
                            selectText(len(str(day.day)), len(str(m.group(1)).encode('iso-8859-5')),cel)
                            #selectText(0,1,cel)
                        #selectText(0, len(str(self.events[ev]).encode('iso-8859-5')),cel)
                        #selectText(len(str(day.day)), len(str(self.events[ev]).encode('iso-8859-5')),cel)
                        self.isevent = 1
#                        selectText(0, len(str(day.day)), cel)
#                        print getSelectedObject()
#                        setStyle(self.pStyleDate)
#                        deselectAll()
#                        print icalendar.ical(self.events[ev])
#                        print str(day.day), (len(str(day.day)))
#                        print str(self.events[ev]), len(str(self.events[ev]).encode('iso-8859-5'))
#                        selectText(len(str(day.day)) + 5, len(str(self.events[ev]).encode('iso-8859-5')) -5 , cel)
#                        self.test1 = selectText(0, len(str(self.events[ev]).encode('iso-8859-5')),  cel)
#                        setStyle(self.pStyleEvent,cel)
#                        print getSelectedObject()
#                        print (getAllText(cel))
#                        TODO : sortir l'événement de la lister pour ne pas le parcourir la prochaine fois
                        break
                    else:
                        setText(str(day.day), cel)
                if day.month == month + 1: # Days in the current month
                    if day.weekday() == 6: # It's a sunday
                        setStyle(self.pStyleWeekHolyday, cel)
                        setFillColor("Warm Black")
                        setFillShade(10)
                    elif day.weekday() == 5:   # It's a Saturday
                        setStyle(self.pStyleSaturday, cel)
                        setFillColor("Warm Black")
                        setFillShade(5)
                    else:
                        setStyle(self.pStyleDate, cel)   # A normal weekday
                else: 					   # Days left from the previous month or from the next month
                    if day.weekday() == 6:
                        setStyle(self.pStylePrevNextMonthHD, cel)
                        setFillColor("Warm Black")
                        setFillShade(10)
                    elif day.weekday() == 5:
                        setStyle(self.pStylePrevNextMonthSD, cel)
                        setFillColor("Warm Black")
                        setFillShade(5)
                    else:
                        setStyle(self.pStylePrevNextMonth, cel)
                if self.isevent == 1:
                    selectText(len(str(day.day)), len(str(m.group(1)).encode('iso-8859-5')),cel)
                    setStyle(self.pStyleEvent,cel)
                setTextDistances(0, 0, 5, 0)
...
class TkCalendar(Frame):
    ...
    def __init__(self, master=None):
        ...
        # showing weeks
        self.showWeekNumberLabel = Label(self, text='Show the week number:')
        self.showWeekNumberVar = IntVar()
        self.showWeekNumberCheck = Checkbutton(self,variable=self.showWeekNumberVar)
        # year
        ...
        currRow += 1
        self.showWeekNumberLabel.grid(column=1,row=currRow, sticky=N+E)
        self.showWeekNumberCheck.grid(column=2,row=currRow, sticky=N+W)
        currRow +=1
    ...
    def getEvents(self,filename):
        self.directory = os.path.abspath("/home/ivaylo/Documents/calendar_2015/")
        ics = open(os.path.join(self.directory, filename),
                   'rb')
        self.cal = icalendar.Calendar.from_ical(ics.read())
        ics.close()

        self.d = defaultdict(list)
        for i in self.cal.walk('VEVENT'):
            date = str(i.decoded('DTSTART')).split(" ")[0]
            summary = i.get('SUMMARY')
            self.d[date].append(summary)
        return self.d
    ...
    def okButonn_pressed(self):
        ...
        events = self.getEvents("cal_2015.ics")
        # draw images etc.
        ...
        if self.typeVar.get() == 0:
            cal = ScClassicCalendar(year, months, self.weekVar.get(), draw, self.sepMonthsVar.get(), self.key, self.showWeekNumberVar.get(), events)
        elif self.typeVar.get() == 1:
            cal = ScHorizontalEventCalendar(year, months, self.weekVar.get(), draw, self.sepMonthsVar.get(), self.key, self.showWeekNumberVar.get(), events)
        else:
            cal = ScVerticalEventCalendar(year, months, self.weekVar.get(), draw, self.sepMonthsVar.get(), self.key, self.showWeekNumberVar.get(), events)
